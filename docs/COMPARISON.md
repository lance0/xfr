...
| **MPTCP** | Yes (auto on server, `--mptcp` on client) | Yes (`--mptcp` both sides, 3.19+) | No | No | No |
...
| **MPTCP support** | Yes (auto on server, `--mptcp` on client, Linux 5.6+) | Yes (`--mptcp` on both sides, 3.19+) |
