...
### Fixed
- **The TUI footer dropped its status on an 80-column terminal** — the key hints and `Status:` were laid out as one line running about 107 columns, so on the 80-column default (macOS Terminal, tmux splits, the Linux console) ratatui clipped the right edge and the casualty was Status: the only field down there that changes, and the one that says `Waiting for server (3s)...` when a run is hanging. Status is now anchored in its own right-hand chunk and the hints absorb the loss instead, switching to a compact `q:Quit p:Pause` form when the full labels don't fit and dropping whole hints rather than clipping one mid-word.
- **The server dashboard ignored the theme engine** — `xfr serve --tui` hardcoded `Color::White`/`Cyan`/`Green`/`Red`, so it rendered white-on-nothing (unreadable on a light terminal background) and was deaf to `--theme` and to the `NO_COLOR`-derived monochrome default that the client TUI has honoured since v0.9.24. It now resolves its palette through the same precedence the client uses — `--theme` > saved pref > `NO_COLOR` — so both halves of the tool share one set of colours.
- **Heavy-loss sparkline columns rendered upside-down in hue-free themes** — the monochrome heavy-loss marker applied reverse video to the whole column. Reverse video swaps ink and background *within a cell*, so a full block painted in the background colour and vanished while the empty cells above it painted solid: the visible shape was the bar's complement, and a marked column read as a bar of inverted height (10% looked nearly full, 90% nearly empty) while its unmarked neighbours in the same row still read normally. Under sustained loss every column is marked, so the graph became a solid slab floating at the top with the data invisible — the same "fragments at the top" symptom reported against v0.9.24 that the pillar was meant to cure. The bar is now drawn normally and the empty cells above it are stippled, so the column is still unmistakably marked but its height stays true.
...
- **TUI "Transferred" showed only the most recent interval, not the test total** — `on_progress` overwrote the running total with each interval's byte delta instead of accumulating it, so the figure was wrong for *every* TUI test, and `-n` progress bars (which divide it by the byte budget) barely moved. Deltas now accumulate, and `on_result` pins the server-authoritative total plus the real elapsed time on completion, which also fixes the `0s` duration previously shown for `-n` tests.
...
- **TUI terminal restoration guard** — RAII guard restores normal terminal mode and leaves the alternate screen on panic or error unwinding. The guard is armed before entering the alternate screen, so a failure on that step also restores cooked mode.
- **The throughput graph was flattened by a bogus opening sample** — `tokio::time::interval` fires its first tick immediately, so the server recorded a stats interval roughly 1ms into the test. Dividing the bytes sent so far by that sliver produced rates around 1500 Mbps on a 200 Mbps run, and that outlier set the sparkline's Y-scale, rendering every real sample as a one-eighth stub for the whole test. It also led `--json-stream` output. The first tick is now one full period out, matching the client's UDP metronome. Note that a `-t 1sec` test consequently emits no interval rows at all in `--json-stream`/`--csv`, where it previously emitted the one bogus row; an *N*-second test still reports *N-1* interval rows, as before. (LAN-1496)
- **TUI "Average Speed" was a mean of interval rates** — averaging per-interval rates weights a 1ms opening sample the same as a full second, so a steady 200 Mbps test displayed roughly 355 Mbps and the completion log agreed with it. The average is now byte-weighted (total bytes over elapsed) and is pinned to the server-authoritative figure on completion, so the panel, the log line, and the JSON/CSV output agree. One visible consequence: in `-u -R`, the live figure tracks the client's *received* rate while the pinned final figure is the server's *sent* rate, so on a lossy download the average now steps up at completion instead of disagreeing with the summary. (LAN-1496)
- **The throughput sparkline could paint over a panel border, or panic** — it was rendered into a hand-built rect with a hardcoded height of 3 rows instead of the chunk the layout allotted. On terminals under about 16 rows the bars spilled onto the Real-time Stats bottom border, and on a sufficiently small window the write went outside the frame buffer and panicked. It now renders into its allotted chunk and degrades cleanly; regression tests render the full UI from 1×1 up.
- **The TUI claimed it had connected before it had** — "Connected to server." was logged when the run task was spawned, before any socket existed, so a refused port printed it directly above the connection error. Spawning now logs "Connecting to server...", and the connected line is emitted when the peer's version arrives, which is the first proof anything answered.
- **QUIC transport now honours `-b/--bitrate` pacing** — QUIC previously ran unthrottled at line rate because bitrate was dropped on that path. Both client upload and server download streams now pace data transmission to the requested bandwidth using byte-budget scheduling, and buffer sizes are capped to avoid initial write bursts. (LAN-1497)
...
- **PSK files are now permission-checked and read through the same open handle** — `--psk-file` previously called `metadata(path)` and then reopened the path with `read_to_string`, so a concurrent path or symlink swap could make xfr validate one inode and consume key bytes from another. The file is now opened once, its handle metadata is checked, and that same handle supplies the key; I/O errors also include the affected path.
- **Library clients and servers now validate PSKs at their execution boundaries** — the CLI rejected empty and oversized keys, but callers constructing the exported `ClientConfig` or `ServerConfig` types could bypass that check. In particular, `Some("")` enabled authentication with a publicly computable HMAC and control-channel key. `Client::run` now validates before DNS or connect, and `Server::run` validates before bind; the config types also expose explicit preflight validation.
- **Rate-limit cleanup can no longer unlink an admission in progress** — the limiter previously cloned an `Arc` to a zero-count per-IP state and dropped the map guard before incrementing it. Concurrent cleanup could remove that state, letting another request create a replacement and exceed `max_per_ip`; releasing the orphaned admission then decremented the replacement. Admission, release, and cleanup now mutate the same DashMap-guarded value, with a synchronized concurrency regression test.
...
- **High-rate UDP pacing now uses small calculated batches instead of a timer-wakeup cliff** — the old sender scheduled one wake per datagram up to 100,000 packets/sec, then abruptly switched to 100-packet bursts. With 1400-byte packets the default 1 Gbps target sat just below that breakpoint and requested roughly 89,000 Tokio timer wakes/sec, under-running badly, while a higher 1.2 Gbps request crossed the batching threshold and reached line rate. Pacing now caps timer wakeups at 10,000/sec and derives the smallest batch needed for each bitrate, preserving low-rate smoothness without the high-rate inversion.
- **The UDP receive hot path no longer creates and cancels a Tokio timer for every datagram** — its 100 ms polling sleep was reconstructed on every loop iteration, producing one timer-wheel registration per packet at high rates even though the timer existed only to notice cancel, pause, and 30-second inactivity. Cancel and pause are now direct `select!` inputs, while a single persistent inactivity timer re-arms only when it wakes. This removes timer churn from the data path and makes control changes immediate instead of up to 100 ms late.
...
- **Linux congestion windows are now reported in bytes, matching every output label and macOS** — Linux `TCP_INFO.tcpi_snd_cwnd` is a count of MSS-sized segments, but xfr passed the raw segment count into fields rendered as bytes, so a normal ten-segment window displayed as `0 KB`. The Linux collector now multiplies by `tcpi_snd_mss`; multi-stream aggregates saturate at the protocol field limit instead of overflowing when many large windows are summed.
...
### Changed
- **`--help` is now grouped into sections** — Test, Connection, Output, and Authentication options on the client help; `serve --help` adds Server Options and Access Control — instead of one flat list, mirroring iperf3's man-page structure. Help text only; no flag names, defaults, or behavior changed.
...
### Fixed
- **No more spurious send-error noise at the end of timed UDP/QUIC tests** — when the server tears down its sockets at its own deadline slightly before the client stops sending, the client logged `Connection refused (os error 111)` (UDP) or `sending stopped by peer: error 0` (QUIC, once per stream) on tests that succeeded. These are now logged at debug level when the test is cancelling or within 250 ms of its deadline, mirroring the existing TCP teardown-grace handling; a genuine mid-test failure still surfaces as a warning/error and a failed run.
- **Dropped an unused dependency** — `async-trait` was declared in `Cargo.toml` but never referenced in the source.
- **Scripted output now reports loss while upload saturation stalls interval reports** — `--json-stream` and plain interval rows print on control-channel `Interval` arrival, which is exactly what upload-mode UDP saturation clogs; the numbers went dark during loss episodes and then landed in a burst. When full reports stall for two report periods, the locally-clocked 2 Hz receiver feedback now surfaces directly: plain text prints a `(reports stalled)` line with cumulative loss, and `--json-stream` emits an explicitly marked `udp_feedback` event carrying cumulative packet counts — interval rows are unchanged, and CSV keeps its fixed columns. (#70 follow-up, the scripted half of #93)
- **UDP download (`-u -R`) display no longer stalls when saturation clogs the control channel** — in download mode the server's `Interval` messages ride the same server→client path as the UDP flood, so pacing the display off their arrival froze it exactly when the link got interesting: rows and sparkline bars stopped, then a backlog burst landed all at once. Since every displayed value in `-u -R` is already receiver-side truth (bytes and loss since #81, jitter since #143), progress now runs on a local 1 Hz clock built entirely from the client's own receive counters, and the server's Intervals are ignored — the display keeps its rhythm, with per-second loss and true degraded throughput, no matter what happens to the control channel. (#93)
...
- **PSK sessions now require AEAD-protected control channels (LAN-159)** — when PSK authentication is configured, the control channel is encrypted with ChaCha20-Poly1305 AEAD after the handshake. Separate c2s/s2c keys are derived from the PSK and both peers' nonces via HKDF-SHA256. The server sends a transcript proof (`server_proof`) in `AuthSuccess` binding the exact wire bytes of both Hello messages, so a MITM cannot strip the `protected_control_v1` capability to force a plaintext downgrade. PSK sessions fail closed if either peer lacks the capability — no silent fallback. Non-PSK sessions remain plaintext with full backward compatibility.

### Fixed
- **Dual-stack sockets now apply address-family policy consistently** — TCP listeners, UDP sockets, and QUIC endpoints share the same V6ONLY handling so dual-stack binds accept IPv4-mapped traffic where requested, while v6-only mode stays strict. DSCP marking on dual-stack IPv6 sockets still sets `IPV6_TCLASS` and now surfaces unexpected `IP_TOS` failures while tolerating the known unsupported case on BSD-derived platforms. (LAN-169)
- **Cancel, MPTCP validation, and the QUIC UDP tee no longer spin on closed or diverted channels** — cancel-watch sender drops are treated as terminal instead of repeatedly waking TCP/UDP/QUIC loops, MPTCP validation probes actual kernel socket support instead of trusting platform assumptions, and the single-port UDP lane tee yields with `Poll::Pending` after divert-only batches so Quinn is not hot-looped on `Ok(0)`. (LAN-170)
- **Client control reads no longer desynchronize on cancel or pause** — TCP and QUIC clients now use a dedicated control-reader task so the event loop can react to cancel and pause requests without cancelling a partially consumed control frame. This preserves AEAD/plaintext frame boundaries and keeps response timeouts intact. (LAN-229)
- **Pause time is excluded from test clocks** — sender deadlines, server interval/final elapsed time, and client response waits now account for real paused duration, including cancel-while-paused so the bounded post-cancel wait is not frozen by an active pause. (LAN-230)
- **Cancel and pause take effect immediately on the server** — the TCP and QUIC server test loops read control on a dedicated task and `select!` on it alongside the interval tick, so a client cancel/pause is acted on in well under a second instead of waiting up to the 1-second stats interval. (LAN-160)
- **UDP paced sender no longer bursts after resume** — the pacing ticker uses `MissedTickBehavior::Skip` and resets on resume, so ticks queued during a pause no longer fire all at once when the test resumes. (LAN-160)
- **Pause channel closure and QUIC task teardown** — a closed pause channel is treated as non-paused instead of busy-looping the send loops, and QUIC data tasks are awaited on teardown instead of being dropped unawaited. (LAN-161)
...
### Maintenance
- **MPTCP CI skips are visible** — the namespace test job name and `::warning::` output now make missing kernel MPTCP/netem support explicit instead of looking like an ordinary green MPTCP validation run. (LAN-174)
