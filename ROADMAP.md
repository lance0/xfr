...
- [x] **Suppress UDP/QUIC send errors on graceful shutdown** (PR #157) - downgraded to debug when the test is cancelling or within 250ms of its deadline (mirrors tcp.rs SEND_TEARDOWN_GRACE); QUIC also treats peer-initiated `Stopped`/`ConnectionLost` near the deadline as clean teardown and no longer fails on `finish()` after a peer stop. Mid-test failures still warn/error
...
- [x] **Feedback-driven scripted output cadence** (#70 follow-up, non-TUI half; PR #154) — when full Interval rows stall for ~two report periods, plain text prints a `(reports stalled)` cumulative-loss line and `--json-stream` emits a marked `udp_feedback` event from the locally-clocked 2 Hz feedback (the "explicit feedback-only events with a schema marker" option); CSV keeps fixed columns. Download mode (`-u -R`) went further in PR #153: interval rows there are built wholly from local receive counters on a local 1 Hz clock, so they never stall at all
...
- [x] **Group CLI help by client/server** (PR #155) — `--help` grouped into Test/Connection/Output/Authentication sections via clap `help_heading`; `serve --help` adds Server Options and Access Control. Help text only, no flag behavior changed
...
- [x] **Clean up remaining dead code** (PR #156) - unused `ProgressBar::style()` removed. `InstallMethod::update_command` turned out to be complete and in use (TUI update prompt) — the earlier note here was stale. Remaining `#[allow(dead_code)]` items are deliberate (multi-port fallback in serve.rs, feature-parity stub in discover.rs); the `ProgressBar` widget itself is public API with no internal callers, kept
- [x] **Add SAFETY comments** - document invariants for 4 unsafe blocks in tcp_info.rs, tcp.rs, net.rs
- [x] **Audit unwrap()/expect() calls** (PR #156) - audit complete: auth.rs HMAC init and the serve.rs/client.rs PSK paths already propagate errors or carry infallibility messages; the one remaining bare unwrap (control_crypto frame-header conversion) now documents why it cannot fail
